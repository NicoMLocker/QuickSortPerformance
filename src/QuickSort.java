...
public class QuickSort {
    ...
    public static void main(String[] args) {
        ...
        runFullExperiment("QuickSort-Exp1.txt");
    ...
    private static long randPartition(long[] list, long lo, long hi){
        long randIndex = randomPivot(lo, hi);
        swap(list, hi, randIndex);
        return partition(list, lo, hi);
    }

    // picks a random pivot
    private static long randomPivot(long lo, long hi){
        return (long)Math.floor(Math.random() * (hi - lo + 1) + lo);
    }

    private static void swap(long[] list, long first, long second){
        long temp = list[(int)first];
        list[(int)first] = list[(int)second];
        list[(int)second] = temp;
    }
}
